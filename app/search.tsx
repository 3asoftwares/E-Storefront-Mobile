...
export default function SearchScreen() {
    ...
    const { data, isLoading, refetch } = useProducts(1, 20, shouldSearch ? { search: debouncedQuery } : undefined);
    ...
    const popularSearches = ['Electronics', 'Clothing', 'Shoes', 'Watches', 'Headphones', 'Laptops', 'Phones', 'Accessories'];
            ...
            {shouldShowProducts ? (
                ...
                <View style={styles.content}>
                    {isLoading ? (
                        ...
                        <EmptyState icon='🔍' title='No results found' description={`We couldn't find anything for "${debouncedQuery}"`} />
                    )}
                </View>
            ) : (
                        ...
                        <View style={styles.section}>
                            ...
                            <View style={styles.chipContainer}>
                                {recentSearches.slice(0, 10).map((query, index) => (
                                    <TouchableOpacity key={index} style={styles.chip} onPress={() => handleRecentSearchPress(query)}>
                    ...
                    <View style={styles.section}>
                        ...
                        <View style={styles.chipContainer}>
                            {popularSearches.map((query, index) => (
                                <TouchableOpacity key={index} style={[styles.chip, styles.popularChip]} onPress={() => handleRecentSearchPress(query)}>
                                    <Text style={styles.chipIcon}>🔥</Text>
                                    <Text style={styles.chipText}>{query}</Text>
                                </TouchableOpacity>
                            ))}
