...
import { Colors, BorderRadius, Spacing, Shadows, FontSizes, FontWeights } from '../../src/constants/theme';
...
function ProductCard({ product, index = 0 }: { product: any; index?: number }) {
    ...
    return (
        <Animated.View
            ...
            <Pressable style={styles.productCard} onPress={() => router.push(`/product/${product.id}`)} onPressIn={handlePressIn} onPressOut={handlePressOut}>
                <View style={styles.productImageContainer}>
                    <Image source={{ uri: imageUrl }} style={styles.productImage} resizeMode='cover' />
                    {hasDiscount && (
                        <LinearGradient colors={[Colors.light.error, '#FF8080']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.discountBadge}>
                            <Text style={styles.discountText}>-{discountPercentage}%</Text>
                        </LinearGradient>
                    )}
                    <Animated.View style={[styles.wishlistButton, { transform: [{ scale: heartAnim }] }]}>
                        <TouchableOpacity onPress={handleWishlistPress} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}>
                            <FontAwesomeIcon
    ...
    return (
        <View style={styles.heroBanner}>
            ...
            <Animated.View style={[styles.heroContent, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>
                ...
                <Text style={styles.heroSubtitle}>Shop the latest trends with exclusive deals up to 50% off</Text>
                <TouchableOpacity style={styles.heroButton} onPress={() => router.push('/products')} activeOpacity={0.9}>
...
function SectionHeader({ title, subtitle, icon, onSeeAll }: { title: string; subtitle?: string; icon?: any; onSeeAll?: () => void }) {
    return (
                ...
                {recentlyViewed.length > 0 && (
                    <View style={styles.section}>
                        ...
                        <FlatList
                            ...
                            renderItem={({ item }) => (
                                <TouchableOpacity style={styles.recentlyViewedCard} onPress={() => router.push(`/product/${item.productId}`)}>
                                    <Image source={{ uri: item.image }} style={styles.recentlyViewedImage} resizeMode='cover' />
